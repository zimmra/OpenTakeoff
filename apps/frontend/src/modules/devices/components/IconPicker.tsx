...
import { mdiChevronDown, mdiChevronUp, mdiClose, mdiFlipHorizontal, mdiFlipVertical } from '@mdi/js';
...
export function IconPicker({ value, onChange, label, error }: IconPickerProps) {
  ...
  const [config, setConfig] = useState<IconConfig>(() => 
    parseIconKey(value) ?? { provider: 'mdi', name: 'ceiling-light', modPos: 'top-right', textPos: 'bottom-right' }
  );
  ...
  const renderIconPreview = (conf: IconConfig, size = 24) => {
    if (conf.provider === 'tabler') {
      ...
      const IconComponent = icons[conf.name] as React.ComponentType<{ size: number; stroke: number }> | undefined;
    ...
    return (
      <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
        ...
        {/* Text */}
        {conf.text && (
          <div className={`absolute font-bold leading-none select-none flex items-center justify-center`} style={textStyle}>
            {conf.text}
          </div>
        )}
      </div>
    ...
    <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-slate-100 rounded border border-slate-200">
                {renderIconPreview({ ...config, modifier: undefined, modifiers: undefined, text: undefined })}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              {activeModifiers.length > 0 && (
                <select
                  ...
                  onChange={(e) => handleConfigChange({ modPos: e.target.value as 'top-right' | 'bottom-right' })}
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100">
              ...
              <div className="flex gap-1">
                 <button
                    ...
                    className={`p-1.5 rounded transition-colors ${
                      config.flipX ? 'bg-primary-100 text-primary-700' : 'hover:bg-slate-100 text-slate-600'
                    }`}
                 ...
                 <button
                    ...
                    className={`p-1.5 rounded transition-colors ${
                      isFlipY ? 'bg-primary-100 text-primary-700' : 'hover:bg-slate-100 text-slate-600'
                    }`}
                    title="Flip Vertical"
                 >
                ...
                <div className="flex items-center justify-between">
                   ...
                   <div className="flex gap-1">
                     <button
                        ...
                        className={`p-1.5 rounded transition-colors ${
                          config.modFlipX ? 'bg-primary-100 text-primary-700' : 'hover:bg-slate-100 text-slate-600'
                        }`}
                     ...
                     <button
                        ...
                        className={`p-1.5 rounded transition-colors ${
                          config.modFlipY ? 'bg-primary-100 text-primary-700' : 'hover:bg-slate-100 text-slate-600'
                        }`}
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <select
                    value={(config.textPos === 'super' || !config.textPos) ? 'top-right' : config.textPos}
                    onChange={(e) => handleConfigChange({ textPos: e.target.value as 'top-right' | 'bottom-right' | 'center' })}
             ...
             {(activeModifiers.length > 0 || config.text) && (
               <div className="space-y-3 pt-2 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {activeModifiers.length > 0 && (
                      <div className="space-y-2">
                        <label className="text-xs text-slate-700 font-medium">Modifier Vertical</label>
                        <div>
                          <input
                            type="range"
                            min="-100"
      ...
      >
        <p className="text-sm text-slate-600">
            Would you like to move the {conflictDetails?.type === 'text-moved' ? 'modifier icon' : 'text label'} to the {conflictDetails?.suggestedSwap.modPos === 'top-right' || conflictDetails?.suggestedSwap.textPos === 'top-right' ? 'top right' : 'bottom right'} corner?
        </p>
      </Dialog>
